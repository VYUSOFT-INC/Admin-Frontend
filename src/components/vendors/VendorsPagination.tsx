...
interface VendorsPaginationProps {
  /** Vendors currently visible after filtering/search. */
  shownCount: number;
  /** Total vendors in the (mock) dataset, unfiltered. */
  totalCount: number;
}

/**
 * Pagination footer. The bundled mock dataset only ships 10 vendor records
 * (see `src/lib/mock-data/vendors.ts`), so — unlike the Figma reference,
 * which implies ~84 records across ~9 pages — this only ever renders a
 * single active page. The prev/next controls are wired but disabled since
 * there is nothing to page through yet.
 */
export function VendorsPagination({ shownCount, totalCount }: VendorsPaginationProps) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-5 py-3.5">
      <p className="text-[12.5px] font-medium text-gray-500">
        Showing <span className="font-bold">{shownCount}</span> of <span className="font-bold">{totalCount}</span> vendors
      </p>
